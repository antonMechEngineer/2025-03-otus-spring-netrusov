...
public class JdbcGenreRepository implements GenreRepository {
    ...
    public List<Genre> findAll() {
        ...
        return namedParameterJdbcOperations.query(query, new GnreRowMapper());
    }

    @Override
    public Optional<Genre> findById(long id) {
        String query = "SELECT id, name FROM genres WHERE id = :id";
        List<Genre> genres = namedParameterJdbcOperations.query(query, Map.of("id", id), new GnreRowMapper());
        if (genres.isEmpty()) {
